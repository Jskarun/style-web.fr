...
class Settings extends Koken_Controller {
	...
	function index()
	{
		...
		$data = array();
		$bools = array('site_hidpi', 'retain_image_metadata', 'image_use_defaults');
...
OUT;
				...
				foreach($_POST as $key => $val)
				{
					if (isset($data[$key]) && $data[$key] !== $val)
					{
						...
						{
							delete_files( FCPATH . DIRECTORY_SEPARATOR . 'storage' . DIRECTORY_SEPARATOR . 'cache' . DIRECTORY_SEPARATOR . 'images', true, 1 );
						}
						$save[$key] = $val;
					}
				}
		...
		if (!isset($data['site_timezone']) || empty($data['site_timezone']))
		{
			$data['site_timezone'] = 'Etc/UTC';
		}
