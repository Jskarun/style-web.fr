<?php
	...
	{
		$referer = preg_replace('~https?://' . $_SERVER['HTTP_HOST'] . $base_folder . '(/(index|preview)\.php\?)?~', '', $_SERVER['HTTP_REFERER']);

		if (strpos($referer, 'http') === 0)
		{
			// Referrer is not from this domain,
			$referer = false;
		}
		else
		{
	...
	if ((!$draft || $preview) && !isset($_GET['default_link']))
	{
		...
		{
			$cache_url = $base_folder . preg_replace('/\?|&|=/', '_', preg_replace('/\?|&_pjax=[^&$]+/', '', urldecode(rtrim($cache_url, '/'))));
			$css = false;
		}
