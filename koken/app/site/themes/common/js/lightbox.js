...
$(function() {
	...
	$(document).on('click', '#lbox-bttn-close', function() {
		...
		if ($K.location.defaults.content) {
			template = '^' + $K.location.defaults.content.replace(/\:[a-z_-]+/g, '([^/]+)') + '$';
			if ($K.location.referer.match(RegExp(template))) {
				location.href = root + $K.location.defaults.content.replace(':id', contentId) + preview;
				return false;
			}
		}
		if ($K.location.defaults.content_in_album) {
			template = '^' + $K.location.defaults.content_in_album.replace(/\:[a-z_-]+/g, '([^/]+)') + '$';
			if ($K.location.referer.match(RegExp(template))) {
				location.href = root + $K.location.defaults.content_in_album.replace(':id', contentId).replace(':album_id', albumId) + preview;
				return false;
			}
		}

		if ($K.location.referer.indexOf('http://') === -1 && !$K.location.rewrite) {
			location.href = root + $K.location.referer + preview;
		} else {
